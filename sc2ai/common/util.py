...
def general_n_step_advantage(
        one_step_rewards: np.ndarray,
        one_step_rewards_modified: np.ndarray,
    ...
    batch_size_mod, timesteps_mod = one_step_rewards_modified.shape
    #assert value_estimates.shape == (batch_size, timesteps + 1)
    assert value_estimates.shape == (batch_size_mod, timesteps_mod+1)
    #delta = one_step_rewards + discount * value_estimates[:, 1:] - value_estimates[:, :-1]
    delta_mod = one_step_rewards_modified + discount * value_estimates[:, 1:] - value_estimates[:, :-1]
    ...
    #delta_rev = delta[:, ::-1]
    delta_rev_mod = delta_mod[:, ::-1]
    #adjustment = (discount * lambda_par) ** np.arange(timesteps, 0, -1)
    adjustment_mod = (discount * lambda_par) ** np.arange(timesteps_mod, 0, -1)
    #advantage = (np.cumsum(delta_rev * adjustment, axis=1) / adjustment)[:, ::-1]
    advantage_mod = (np.cumsum(delta_rev_mod * adjustment_mod, axis=1) / adjustment_mod)[:, ::-1]
